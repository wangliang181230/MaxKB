...
class GeneratePromptSerializers(serializers.Serializer):
    ...
    def is_valid(self, *, raise_exception=False):
        ...
        messages = self.data.get("messages")  # 第一次使用 self.data，后面直接用缓存 self._data
...
class DebugChatSerializers(serializers.Serializer):
    ...
    def chat(self, instance: dict, base_to_response: BaseToResponse = SystemToResponse()):
        ...
        chat_id = self.data.get('chat_id')  # 第一次使用 self.data，后面直接用缓存 self._data
...
class PromptGenerateSerializer(serializers.Serializer):
    ...
    def is_valid(self, *, raise_exception=False):
        super().is_valid(raise_exception=True)
        workspace_id = self.data.get('workspace_id')  # 第一次使用 self.data，后面直接用缓存 self._data
        query_set = QuerySet(Application).filter(id=self._data.get('application_id'))
        if workspace_id:
    ...
    def generate_prompt(self, instance: dict):
        application = self.is_valid(raise_exception=True)  # 第一次使用 self.data，后面直接用缓存 self._data
        GeneratePromptSerializers(data=instance).is_valid(raise_exception=True)
        workspace_id = self._data.get('workspace_id')
...
class OpenAIChatSerializer(serializers.Serializer):
    ...
    def chat(self, instance: Dict, with_valid=True):
        ...
        application_id = self.data.get('application_id')  # 第一次使用 self.data，后面直接用缓存 self._data
        chat_user_id = self._data.get('chat_user_id')
        chat_user_type = self._data.get('chat_user_type')
        ip_address = self._data.get('ip_address')
...
class ChatSerializers(serializers.Serializer):
    ...
    def is_valid_chat_id(self, chat_info: ChatInfo):
        if self._data.get('application_id') is not None and self._data.get('application_id') != str(
                chat_info.application_id):
            raise ChatException(500, _("Conversation does not exist"))
        if self._data.get('chat_user_id') is not None and self._data.get('chat_user_id') != str(
                chat_info.chat_user_id):
            raise ChatException(500, _("Conversation does not exist"))

    def is_valid_intraday_access_num(self):
        if not self._data.get('debug') and [ChatUserType.ANONYMOUS_USER.value,
                                           ChatUserType.CHAT_USER.value].__contains__(
            self._data.get('chat_user_type')):
            access_client = QuerySet(ApplicationChatUserStats).filter(chat_user_id=self._data.get('chat_user_id'),
                                                                      application_id=self._data.get(
                                                                          'application_id')).first()
            if access_client is None:
                access_client = ApplicationChatUserStats(chat_user_id=self._data.get('chat_user_id'),
                                                         chat_user_type=self._data.get('chat_user_type'),
            ...
            application_access_token = QuerySet(ApplicationAccessToken).filter(
                application_id=self._data.get('application_id')).first()
    ...
    def chat_simple(self, chat_info: ChatInfo, instance, base_to_response):
        ...
        pipeline_message = (pipeline_manage_builder.append_step(BaseSearchDatasetStep)
                            ...
                            .add_debug(self._data.get('debug', False))
                            .build())
        ...
        work_flow_manage = WorkflowManage(Workflow.new_instance(work_flow),
                                          {
                                              ...
                                              'chat_user': {
                                                  ...
                                                  "source": chat_info.get_chat_user().get("source"),
                                              } if chat_user_type == ChatUserType.CHAT_USER.value else chat_info.get_chat_user(),
    ...
    def chat(self, instance: dict, base_to_response: BaseToResponse = SystemToResponse()):
        ...
        chat_info = self.get_chat_info()  # 第一次使用 self.data，后面直接用缓存 self._data

        # 如果会话用户类型为 CHAT_USER，且当前请求用户类型为 APPLICATION_API_KEY，则使用会话
        if (chat_info.chat_user_type == ChatUserType.CHAT_USER.value
                and self._data.get("chat_user_type") == ChatUserType.APPLICATION_API_KEY.value):
            self._data.get("source", {})["api_key_id"] = self._data.get("chat_user_id")  # 保存原始调用的api_key_id
            self._data["chat_user_id"] = chat_info.chat_user_id
            self._data["chat_user_type"] = chat_info.chat_user_type
    ...
    def re_open_chat_simple(self, chat_id, application: Application, chat: Chat):
        ...
        chat_info = ChatInfo(chat_id, chat.chat_user_id, str(chat.chat_user_type),
                             self._data.get('ip_address'),
                             self._data.get('source'), knowledge_id_list,
...
class OpenChatSerializers(serializers.Serializer):
    ...
    def open(self):
        self.is_valid(raise_exception=True)  # 第一次使用 self.data，后面直接用缓存 self._data
        application_id = self._data.get('application_id')
        application = QuerySet(Application).get(id=application_id)
        debug = self._data.get("debug")
        if not debug:
    ...
    def open_work_flow(self, application, chat_user_id=None, chat_user_type=None):
        self.is_valid(raise_exception=True)
        application_id = self._data.get('application_id')
        chat_user_id = chat_user_id or self._data.get("chat_user_id")
        chat_user_type = chat_user_type or self._data.get("chat_user_type", ChatUserType.CHAT_USER.value)
        ip_address = self._data.get("ip_address")
        source = self._data.get("source")
        debug = self._data.get("debug")
