...
class ChatSerializers(serializers.Serializer):
    ...
    def is_valid_chat_id(self, chat_info: ChatInfo):
        if self.data.get('application_id') is not None and str(self.data.get('application_id')) != str(
                chat_info.application_id):
            raise ChatException(500, _("Conversation does not exist"))
        # if self.data.get('chat_user_id') is not None and str(self.data.get('chat_user_id')) != str(
