...
- 确保优化后的问题准确反映原始问题意图，不得改变原意。`,
    ...
    a2aNode: {
      label: 'A2A 代理调用',
      text: '通过标准化协议调用其他智能体，支持子代理委派和代理团队协作',
      agentName: 'Agent 名称',
      agentNamePlaceholder: '请输入子Agent的名称',
      agentUrl: 'Agent URL',
      agentUrlPlaceholder: '请输入Agent的API端点URL',
      protocolType: '通信协议',
      streaming: '支持流式',
      comingSoon: '即将推出',
      collaborationMode: '协作模式',
      subagentMode: '子代理（委派式）',
      subagentTooltip: '适用于边界清晰的单一任务委派',
      teamMode: '代理团队（协作式）',
      teamTooltip: '适用于多角色动态协商的复杂任务',
      apiKey: 'API Key',
      apiKeyPlaceholder: '可选的身份验证密钥',
      advancedConfig: '高级配置',
      timeout: '超时时间（秒）',
      timeoutTip: '单个请求的最大等待时间',
      maxRetries: '最大重试次数',
      enableStreaming: '启用流式传输',
      streamingTip: 'HTTP+SSE协议支持实时流式数据透传',
      inputParams: '输入参数',
      inputParamsTip: '配置要传递给子Agent的参数，点击"添加参数"按钮新增',
      addParam: '添加参数',
      paramKeyPrompt: '请输入参数名称',
      paramKeyPlaceholder: '输入参数名...',
      paramEditTip: '按 Enter 确认，Esc 取消',
      paramExists: '该参数已存在',
      contextFields: '上下文字段',
      selectContextFields: '选择要传递的上下文字段',
      contextFieldsTip: '这些字段的值将传递给子Agent',
      fallbackStrategy: '降级策略',
      fallbackStrategyLabel: '故障处理策略',
      fallbackErrorMessage: '返回错误信息',
      fallbackSkip: '跳过并继续',
      fallbackDefaultValue: '使用默认值',
      defaultValueLabel: '默认值',
      defaultValuePlaceholder: '当Agent调用失败时使用的默认值',
      traceId: '追踪ID',
