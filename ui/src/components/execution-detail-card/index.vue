            ...
            <div class="card-never border-r-6 mt-8" v-if="data.branch_details && data.branch_details.length > 0" v-for="(branch, bIndex) in data.branch_details" :key="bIndex">
              ...
              <div v-if="branch.conditions && branch.conditions.length > 0" class="border-t-dashed lighter clean" style="padding: 0 12px 8px 12px">
                <div v-for="(cond, cIndex) in branch.conditions" :key="cIndex" class="mt-8">
                  <el-tag type="warn" size="small" class="f_val" :title="formatTitle(cond.origin_field_value, cond.field_value, cond.field_type)">
                  ...
                  <el-tag type="warn" size="small" class="f_val"
                    :title="formatTitle(cond.origin_target_value, cond.target_value, cond.target_type)"
            ...
            <div class="card-never border-r-6 mt-8">
              ...
              <div class="p-8-12 border-t-dashed lighter pre-wrap">
                <template v-if="Array.isArray(data.question)">
                  <!-- 先显示图片列表 -->
                  <template v-for="(item, qIndex) in getImageList(data.question)" :key="`img-${qIndex}`">
                    <el-image
                  ...
                  <!-- 再显示视频和文本 -->
                  <div v-for="(item, qIndex) in data.question" :key="`video-and-text-${qIndex}`">
                    <video
                      v-if="item.type === 'video_url'"
                    ...
                    <div v-else-if="item.type !== 'image_url'" class="mb-8">{{ item }}</div>
                  </div>
                </template>
                <template v-else>
                  {{ data.question || '-' }}
...
const downloadFile = (file: any) => {
  if (file && (file.url || file.file_id || file.id)) {
    const url = file.url || `./oss/file/${file.file_id || file.id}`
...
// 格式化title显示
const formatTitle = (origin_value: any, value: any, value_type: any): string => {
  let pre_str = ""
  ...
  return pre_str + formatValue(value, value_type)
}
